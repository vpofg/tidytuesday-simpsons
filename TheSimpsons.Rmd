...
```{r}
ggplot(ep10_per_season, aes(x = factor(season), y = us_viewers_in_millions, fill = season)) +
  geom_bar(stat = "identity") +
...
```{r}
episode_words <- simpsons_episodes %>%
  unnest_tokens(word, title) %>%
  count(word, sort = TRUE) %>%
  filter(nchar(word) > 3)

# Create the word cloud
wordcloud2(episode_words, color = "darkblue", backgroundColor = "lightblue", size = 0.5)
```
...
# Create the word cloud
wordcloud2(location_names, color = "darkblue", backgroundColor = "lightblue", size = 0.5)
```
